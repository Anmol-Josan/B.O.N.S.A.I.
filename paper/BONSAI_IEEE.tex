...
\title{BONSAI: Adaptive Task-Graph Functional Replay for Low-Forgetting Continual Learning}
...
overwrite previously useful input-to-output functions. We present a new
BONSAI variant, Adaptive Task-Graph Functional Replay (ATGFR), that combines a
variational information bottleneck, cached task-graph relations, hierarchical
retrieval, a bounded local positive-definite metric, shared low-rank task
adapters, and function-level replay. ATGFR keeps a class-balanced coreset from
training inputs only, stores the completed task's logits and latent features,
weights replay by task-graph similarity, and increases replay pressure only
when measured old-feature drift exceeds a budget. This design directly targets
the failure mode of parameter-only consolidation: stable coordinates do not
guarantee a stable function. On a four-task, twelve-class structured-image
stream evaluated over five fixed seeds, the matched diagonal control obtains
0.1000 task-aware accuracy and 0.3778 peak-before-final forgetting, whereas
ATGFR obtains 0.6333 accuracy and -0.2000 forgetting after mass-preserving
relation normalization. Task-free accuracy is 0.6333, with 0.5667 mean
task-route accuracy and 0.498 times model-parameter memory. A predeclared
robustness grid spanning two and eight tasks and eight and 128 input features
improves in all four cells. We additionally evaluate ER, DER++, ER-ACE, and
ATGFR on Split-CIFAR-100 using one identical compact convolutional backbone,
exemplar count, optimizer, and three predeclared task orders (two seeds per
order). ATGFR reaches 0.0358 accuracy versus 0.0321 for ER, with 0.0037
forgetting versus 0.0038 for ER, but remains far from a strong CIFAR-100
classifier under the deliberately small training budget. A factorial replay
ablation, wall-clock accounting, and task-order sensitivity analysis expose
which components are and are not supported by the data.
\end{abstract}
...
\item An end-to-end modular \BONSAI{} implementation with a variational
information bottleneck (VIB), cached sliced-Wasserstein and zero-dimensional
persistence descriptors, hierarchical retrieval, local SPD geometry, and
shared low-rank task adapters.
\item A non-cherry-picked evaluation: five seeds for the primary image stream,
all four cells of a task/feature robustness grid, repository scaling from 4 to
50 tasks, and a corrected peak-before-final forgetting metric.
\item A matched real-image mechanism study on Split-CIFAR-100 against ER,
DER++, and ER-ACE, with the same backbone, exemplar count, task orders,
optimizer budget, wall-clock accounting, and scalar-memory accounting.
\item A factorial ablation that removes labels, logits, features, OT, H0
persistence, Euclidean replacement, and the drift thermostat, with all cells
retained even when they weaken the result.
\item A quantitative comparison on the current VIB/MLP prediction core
against \EWC{}, \SI{}, \PackNet{}, and \PNN{}, with the same task stream,
optimizer budget, seeds, and explicit task-free scope.
\end{enumerate}
